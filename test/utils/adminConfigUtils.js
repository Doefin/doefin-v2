...
async function setFeeReceiver({ adminConfig, feeReceiver, caller }) {
    return adminConfig.connect(caller).setFeeReceiver(feeReceiver.address || feeReceiver);
}
...
async function getFees(adminConfig) {
    const [receiver, resolutionFeeBps, makerBps, takerBps] = await adminConfig.getFees();
    return {
        receiver,
        resolutionFeeBps,
        makerBps,
