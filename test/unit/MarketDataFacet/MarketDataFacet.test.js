...
describe("MarketDataFacet", function () {
    ...
    describe("getMarketMetadata", function () {
        ...
        it("should return same metadata for both positions in same condition", async () => {
            ...
            expect(yesMetadata.parentCollectionId).to.equal(noMetadata.parentCollectionId);
            expect(yesMetadata.positionIds.length).to.equal(noMetadata.positionIds.length);
            expect(yesMetadata.partitions.length).to.equal(noMetadata.partitions.length);
        });
    });

    describe("getMarketMetadataByCondition", function () {
        it("should return metadata directly by condition ID", async () => {
            const metadata = await marketDataFacet.getMarketMetadataByCondition(conditionId);
        ...
        it("should return same data as getMarketMetadata", async () => {
            const metadataByCondition = await marketDataFacet.getMarketMetadataByCondition(conditionId);
            const metadataByPosition = await marketDataFacet.getMarketMetadata(yesId);
            
            expect(metadataByCondition.collateralToken).to.equal(metadataByPosition.collateralToken);
            expect(metadataByCondition.parentCollectionId).to.equal(metadataByPosition.parentCollectionId);
            expect(metadataByCondition.positionIds.length).to.equal(metadataByPosition.positionIds.length);
            expect(metadataByCondition.partitions.length).to.equal(metadataByPosition.partitions.length);
        });
    ...
    describe("getComplement", function () {
        ...
        it("should be symmetric (complement of complement is original)", async () => {
            ...
            const complementOfComplement = await marketDataFacet.getComplement(yesComplement);
    ...
    describe("getPositionInfo", function () {
        it("should return all position information in one call", async () => {
            const [returnedConditionId, collateralToken, unit, complementId, metadata] = 
        ...
        it("should be consistent with individual function calls", async () => {
            ...
            const individualCollateralToken = await marketDataFacet.getCollateralToken(yesId);
            ...
            expect(metadata.collateralToken).to.equal(individualMetadata.collateralToken);
        });
    ...
    describe("Integration with multiple conditions", function () {
        ...
        it("should handle multiple conditions correctly", async () => {
            ...
            const markets2 = await marketDataFacet.getMarketsByCondition(secondConditionId);
        ...
        it("should return correct complements within same condition only", async () => {
            const yesComplement = await marketDataFacet.getComplement(yesId);
            const secondYesComplement = await marketDataFacet.getComplement(secondYesId);
