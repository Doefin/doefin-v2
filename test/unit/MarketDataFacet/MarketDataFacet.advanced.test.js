...
describe("MarketDataFacet - Advanced Test Cases", function () {
  ...
  describe("Edge Cases and Error Handling", function () {
    ...
    it("should handle zero position ID", async () => {
      await expect(marketDataFacet.getMarketMetadata(0)).to.be.revertedWith(
        "InvalidPositionId()"
      );
    });
    ...
    it("should return consistent data across multiple calls", async () => {
      ...
      expect(metadata1.positionIds.length).to.equal(
        metadata2.positionIds.length
      );
      expect(metadata2.positionIds.length).to.equal(
        metadata3.positionIds.length
      );
    });
  ...
  describe("Multi-Collateral Token Scenarios", function () {
    it("should handle conditions with different collateral tokens", async () => {
      ...
      await conditionManagerFacet
        .connect(owner)
        .createCondition(oracle.address, questionId1, 2, "ipfs://multi-1");
      ...
      await conditionManagerFacet
        .connect(owner)
        .createCondition(oracle.address, questionId2, 2, "ipfs://multi-2");
      ...
      const collateral1 = await marketDataFacet.getCollateralToken(
        positionIds1[0]
      );
      const collateral2 = await marketDataFacet.getCollateralToken(
        positionIds2[0]
      );
    ...
    it("should return correct metadata for positions with different collaterals", async () => {
      ...
      await conditionManagerFacet
        .connect(owner)
        .createCondition(oracle.address, questionId1, 2, "ipfs://meta-1");
      await conditionManagerFacet
        .connect(owner)
        .createCondition(oracle.address, questionId2, 2, "ipfs://meta-2");
      ...
      const metadata1 = await marketDataFacet.getMarketMetadata(
        positionIds1[0]
      );
      const metadata2 = await marketDataFacet.getMarketMetadata(
        positionIds2[0]
      );
  ...
  describe("Complex Condition Scenarios", function () {
    ...
    it("should handle partial position splits", async () => {
      ...
      const conditionIdPartial = getConditionId(
        oracle.address,
        questionIdPartial,
        4
      );
      ...
      const complement1 = await marketDataFacet.getComplement(
        partialPositionIds[0]
      );
      const complement2 = await marketDataFacet.getComplement(
        partialPositionIds[1]
      );
  ...
  describe("Performance and Scalability Tests", function () {
    it("should handle multiple conditions efficiently", async () => {
      ...
      for (let i = 0; i < numConditions; i++) {
        ...
        await conditionManagerFacet
          .connect(owner)
          .createCondition(oracle.address, questionId, 2, `ipfs://perf-${i}`);
      ...
      console.log(
        `Batch query time for ${numConditions} conditions: ${
          endTime - startTime
        }ms`
      );
  ...
  describe("Integration with Other Facets", function () {
    it("should provide consistent data with ConditionManagerFacet", async () => {
      ...
      const integrationConditionId = getConditionId(
        oracle.address,
      ...
      const returnedConditionId = await marketDataFacet.getConditionId(
        integrationPositionIds[0]
      );
    ...
    it("should handle position queries after condition resolution", async () => {
      ...
      const resolvedConditionId = getConditionId(
        oracle.address,
        resolvedQuestionId,
        2
      );
      ...
      await conditionalFacet
        .connect(oracle)
        .reportPayouts(resolvedQuestionId, [1, 0]);

      // MarketDataFacet should still return valid data
      const metadata = await marketDataFacet.getMarketMetadata(
        resolvedPositionIds[0]
      );
      const collateralToken = await marketDataFacet.getCollateralToken(
        resolvedPositionIds[0]
      );
      const complement = await marketDataFacet.getComplement(
        resolvedPositionIds[0]
      );
  ...
  describe("Data Consistency and Validation", function () {
    it("should maintain referential integrity across all functions", async () => {
      ...
      expect(directMetadata.collateralToken).to.equal(
        conditionMetadata.collateralToken
      );
      expect(directMetadata.collateralToken).to.equal(positionInfo[1]);

      // All should return consistent position arrays
      expect(directMetadata.positionIds.length).to.equal(
        conditionMetadata.positionIds.length
      );
      expect(directMetadata.partitions.length).to.equal(
        conditionMetadata.partitions.length
      );
    ...
    it("should handle boundary conditions gracefully", async () => {
      ...
      const [condId, collateral, unit, complement, metadata] =
