...
describe("ConditionalTokensFacet Advanced", function () {
    ...
    let diamondAddress, conditionalTokensFacet, conditionManagerFacet, accessControlFacet, adminConfigFacet, erc1155Facet;
    let mockToken, mockToken2;
    let snapshotId;

    before(async function () {
        [owner, oracle, oracle2, user1, user2, marketMaker] = await ethers.getSigners();
        ...
        await adminConfigFacet.connect(owner).addCollateralToken(mockToken.address, ethers.utils.parseEther("1"));
        await adminConfigFacet.connect(owner).addCollateralToken(mockToken2.address, ethers.utils.parseEther("0.1"));
    ...
    describe("Condition Preparation", function () {
        it("should prepare condition with owner authorization", async () => {
            ...
            await expect(
                conditionalTokensFacet.connect(owner).prepareCondition(
                    oracle.address,
                    questionId,
                    outcomeSlotCount
                )
            ).to.emit(conditionalTokensFacet, "ConditionPreparation")
            .withArgs(expectedConditionId, oracle.address, questionId, outcomeSlotCount);
        });
        ...
        it("should revert if non-owner tries to prepare condition", async () => {
            ...
            await expect(
                conditionalTokensFacet.connect(user1).prepareCondition(
                    oracle.address,
                    questionId,
                    outcomeSlotCount
                )
    ...
    describe("Payout Reporting", function () {
        ...
        beforeEach(async () => {
            testConditionId = getConditionId(oracle.address, testQuestionId, testOutcomeSlotCount);
            await conditionalTokensFacet.connect(owner).prepareCondition(
                oracle.address,
                testQuestionId,
        ...
        it("should report payouts with valid data", async () => {
            ...
            await expect(
                conditionalTokensFacet.connect(oracle).reportPayouts(testQuestionId, payouts)
            ).to.emit(conditionalTokensFacet, "ConditionResolution")
            .withArgs(testConditionId, oracle.address, testQuestionId, testOutcomeSlotCount, payouts);

            // Verify payouts are stored
            const storedPayouts = await conditionalTokensFacet.getPayoutNumerators(testConditionId);
        ...
        it("should report partial payouts", async () => {
            ...
            await conditionalTokensFacet.connect(oracle).reportPayouts(testQuestionId, payouts);
        ...
        it("should revert with too many payouts", async () => {
            ...
            await expect(
                conditionalTokensFacet.connect(oracle).reportPayouts(testQuestionId, tooManyPayouts)
            ).to.be.revertedWith("InvalidPayoutLength()");
        });
        ...
        it("should revert if condition not prepared", async () => {
            ...
            await expect(
                conditionalTokensFacet.connect(oracle).reportPayouts(unpreparedQuestionId, payouts)
        ...
        it("should revert with all zero payouts", async () => {
            ...
            await expect(
                conditionalTokensFacet.connect(oracle).reportPayouts(testQuestionId, zeroPayouts)
        ...
        it("should handle maximum payout values", async () => {
            ...
            await conditionalTokensFacet.connect(oracle).reportPayouts(testQuestionId, maxPayouts);
    ...
    describe("Position Splitting", function () {
        ...
        beforeEach(async () => {
            ...
            await mockToken.connect(user1).approve(diamondAddress, ethers.utils.parseEther("100"));
        });
        ...
        it("should split position into conditional tokens", async () => {
            ...
            await expect(
                conditionalTokensFacet.connect(user1).splitPosition(
                    mockToken.address,
            ...
            .withArgs(user1.address, mockToken.address, ethers.constants.HashZero, testConditionId, partition, amount);

            // Verify conditional tokens were minted
            const yesCollectionId = await getCollectionId(ethers.constants.HashZero, testConditionId, 1, ethers.provider);
            const noCollectionId = await getCollectionId(ethers.constants.HashZero, testConditionId, 2, ethers.provider);
            ...
            expect(await erc1155Facet.balanceOf(user1.address, yesPositionId)).to.equal(amount);
            expect(await erc1155Facet.balanceOf(user1.address, noPositionId)).to.equal(amount);
        });
        ...
        it("should split position with multiple outcomes", async () => {
            ...
            const multiConditionId = getConditionId(oracle.address, multiOutcomeQuestionId, multiOutcomeSlotCount);
            ...
            for (let i = 0; i < partition.length; i++) {
                const collectionId = await getCollectionId(ethers.constants.HashZero, multiConditionId, partition[i], ethers.provider);
                const positionId = getPositionId(mockToken.address, collectionId);
                expect(await erc1155Facet.balanceOf(user1.address, positionId)).to.equal(amount);
            }
        });
        ...
        it("should handle nested position splitting", async () => {
            ...
            const parentConditionId = getConditionId(oracle.address, parentQuestionId, outcomeSlotCount);
            const childConditionId = getConditionId(oracle2.address, childQuestionId, outcomeSlotCount);

            // Prepare both conditions
            await conditionalTokensFacet.connect(owner).prepareCondition(oracle.address, parentQuestionId, outcomeSlotCount);
            await conditionalTokensFacet.connect(owner).prepareCondition(oracle2.address, childQuestionId, outcomeSlotCount);

            const amount = ethers.utils.parseEther("8");

            // First split on parent condition
            await conditionalTokensFacet.connect(user1).splitPosition(
                mockToken.address,
                ethers.constants.HashZero,
                parentConditionId,
                amount,
            ...
            const parentCollectionId = await getCollectionId(ethers.constants.HashZero, parentConditionId, 1, ethers.provider);
            ...
            .withArgs(user1.address, mockToken.address, parentCollectionId, childConditionId, [1, 2], amount);
        });
    });
    ...
    describe("Position Merging", function () {
        ...
        it("should merge positions back to collateral", async () => {
            ...
            await expect(
                conditionalTokensFacet.connect(user1).mergePositions(
                    mockToken.address,
                    ethers.constants.HashZero,
                    testConditionId,
                    partition,
            ...
            expect(await erc1155Facet.balanceOf(user1.address, yesPositionId)).to.equal(0);
            expect(await erc1155Facet.balanceOf(user1.address, noPositionId)).to.equal(0);
        });
        ...
        it("should merge partial amounts", async () => {
            ...
            await conditionalTokensFacet.connect(user1).mergePositions(
                mockToken.address,
                ethers.constants.HashZero,
                testConditionId,
                partition,
            ...
            expect(await erc1155Facet.balanceOf(user1.address, yesPositionId)).to.equal(mergeAmount);
            expect(await erc1155Facet.balanceOf(user1.address, noPositionId)).to.equal(mergeAmount);
        });
    });
    ...
    describe("Position Redemption", function () {
        ...
        beforeEach(async () => {
            ...
            await conditionalTokensFacet.connect(oracle).reportPayouts(testQuestionId, [1, 0]);
        });
        ...
        it("should redeem winning positions", async () => {
            ...
            await expect(
                conditionalTokensFacet.connect(user1).redeemPositions(
                    mockToken.address,
                    ethers.constants.HashZero,
                    testConditionId,
                    indexSets
        ...
        it("should handle losing positions with zero payout", async () => {
            ...
            await conditionalTokensFacet.connect(user1).redeemPositions(
                mockToken.address,
                ethers.constants.HashZero,
                testConditionId,
                indexSets
        ...
        it("should revert if condition not resolved", async () => {
            ...
            const unresolvedConditionId = getConditionId(oracle.address, unresolvedQuestionId, 2);
    ...
    describe("Utility Functions", function () {
        ...
        it("should return correct collection ID", async () => {
            ...
            const expectedCollectionId = await getCollectionId(parentCollectionId, conditionId, indexSet, ethers.provider);
    ...
    describe("Complex Scenarios", function () {
        it("should handle full lifecycle with multiple users", async () => {
            ...
            await conditionalTokensFacet.connect(user2).splitPosition(
                mockToken.address,
                ethers.constants.HashZero,
                conditionId,
                amount,
            ...
            await conditionalTokensFacet.connect(oracle).reportPayouts(questionId, [0, 1, 0]);
            ...
            await conditionalTokensFacet.connect(user2).redeemPositions(
                mockToken.address,
                ethers.constants.HashZero,
                conditionId,
                [2]
            ...
            const collectionId = await getCollectionId(ethers.constants.HashZero, conditionId, 2, ethers.provider);
            const positionId = getPositionId(mockToken.address, collectionId);

            expect(await erc1155Facet.balanceOf(user1.address, positionId)).to.equal(0);
            expect(await erc1155Facet.balanceOf(user2.address, positionId)).to.equal(0);
        });
        ...
        it("should handle cross-collateral operations", async () => {
            ...
            const collection1 = await getCollectionId(ethers.constants.HashZero, conditionId, 1, ethers.provider);
            ...
            expect(await erc1155Facet.balanceOf(user1.address, position1Token1)).to.equal(amount1);
            expect(await erc1155Facet.balanceOf(user1.address, position1Token2)).to.equal(amount2);
        });
    });
});