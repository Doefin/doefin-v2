...
describe("MarketExecutionFacet limit order matching", function () {
    ...
    let diamondAddress, exchangeFacet, matchExecutionFacet, erc20, erc1155, conditionalFacet, conditionManagerFacet, adminConfig;
    let yesId, noId;
    ...
    before(async function () {
        ...
        await mintAndApproveERC20({ token: erc20, minter: owner, to: taker, amount: mintAmount, spender: diamondAddress });
        ...
        await sendAndApproveERC1155({ token: erc1155, sender: owner, to: maker1, tokenId: yesId, spender: diamondAddress, amount: maker1Amount });
        await sendAndApproveERC1155({ token: erc1155, sender: owner, to: maker2, tokenId: yesId, spender: diamondAddress, amount: maker2Amount });
    });
    ...
    it("matches two crossing limit orders and removes them when filled", async () => {
        ...
        const tx = await matchExecutionFacet
            .connect(taker)
            .fillLimitOrders(takerOrderId, [makerOrderId]);

        await expect(tx)
            .to.emit(matchExecutionFacet, "OrderCompletelyFilled")
            .withArgs(takerOrderId, taker.address, maker1.address, amount, takerExpectedPrice);
        await expect(tx)
            .to.emit(matchExecutionFacet, "OrderCompletelyFilled")
            .withArgs(makerOrderId, maker1.address, taker.address, amount, takerExpectedPrice);
    ...
    it("batch matches taker order across multiple makers and updates remaining orders", async () => {
        ...
        const tx = await matchExecutionFacet
            .connect(taker)
            .fillLimitOrders(takerOrderId, [maker1OrderId, maker2OrderId]);
        ...
        await expect(tx)
            .to.emit(matchExecutionFacet, "OrderCompletelyFilled")
            .withArgs(maker1OrderId, maker1.address, taker.address, maker1Amount, takerExpectedPrice1);
        ...
        await expect(tx)
            .to.emit(matchExecutionFacet, "OrderCompletelyFilled")
            .withArgs(takerOrderId, taker.address, maker2.address, takerAmount, takerExpectedPrice2);
        ...
        expect(maker2Order.remainingAmount).to.equal(maker2Amount.sub(ethers.utils.parseUnits("2", erc20Decimals)));
