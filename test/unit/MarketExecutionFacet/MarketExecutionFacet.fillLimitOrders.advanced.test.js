...
describe("MarketExecutionFacet - fillLimitOrders Advanced Tests", function () {
    ...
    let diamondAddress, exchangeFacet, marketExecutionFacet, erc20, erc1155, conditionalFacet, conditionManagerFacet, adminConfig;
    let yesId, noId;
    ...
    before(async function () {
        ...
        await addCollateralToken({ adminConfig, token: erc20, unit: ercUnit, caller: owner });
        ...
        for (const user of users) {
            await mintAndApproveERC20({ token: erc20, minter: owner, to: user, amount: mintAmount, spender: diamondAddress });
        }
        ...
        for (const user of [maker1, maker2, maker3, taker1, taker2]) {
            await sendAndApproveERC1155({ token: erc1155, sender: owner, to: user, tokenId: yesId, spender: diamondAddress, amount: tokenAmount });
            await sendAndApproveERC1155({ token: erc1155, sender: owner, to: user, tokenId: noId, spender: diamondAddress, amount: tokenAmount });
        }
    });
    ...
    describe("Stress Testing", function () {
        it("should handle maximum number of maker orders efficiently", async () => {
            ...
            expect(receipt.gasUsed.lt(ethers.utils.parseUnits("30", erc20Decimals))).to.be.true; // < 15M gas
        });
        ...
        it("should handle very large order amounts", async () => {
            ...
            const takerLargeAmount = ethers.utils.parseUnits("1000000", erc20Decimals)
            const makerLargeAmount = ethers.utils.parseUnits("1000000", erc20Decimals)
            await mintAndApproveERC20({ token: erc20, minter: owner, to: taker1, amount: takerLargeAmount, spender: diamondAddress });
            await mintAndApproveERC20({ token: erc20, minter: owner, to: owner, amount: largeMintAmount, spender: diamondAddress });


            // Split large amount of collateral
            const [largePosIds, largePositionAmounts] = await splitConditionAndGetPositionIds({
                user: owner,
                amount: largeAmount,
            ...
            await sendAndApproveERC1155({ token: erc1155, sender: owner, to: maker1, tokenId: largeYesId, spender: diamondAddress, amount: largeAmount });
            
            console.log("Maker ERC1155 Balance Before:", await erc1155.balanceOf(maker1.address, largeYesId));
            const makerOrderId = await exchangeFacet.getNextOrderId();
            ...
            console.log("Maker ERC1155 Balance After:", await erc1155.balanceOf(maker1.address, largeYesId));

            console.log("Taker ERC20 Balance Before:", await erc20.balanceOf(taker1.address));
            ...
            console.log("Taker ERC20 Balance After:", await erc20.balanceOf(taker1.address));


            await expect(
                marketExecutionFacet.connect(maker2).fillLimitOrders(takerOrderId, [makerOrderId])
        ...
        it("should handle precision edge cases with small amounts", async () => {
            ...
            await expect(
                marketExecutionFacet.connect(taker1).fillLimitOrders(takerOrderId, [makerOrderId])
            ).to.not.be.reverted;
        });
    });
    ...
    describe("Complex Matching Scenarios", function () {
        it("should handle cascading partial fills across multiple orders", async () => {
            ...
            expect(maker3Order.remainingAmount).to.equal(amounts[2].sub(ethers.utils.parseUnits("5", erc20Decimals)));
        });
        ...
        it("should handle mixed match types in single execution", async () => {
            ...
            const takerYesBalanceBefore = await erc1155.balanceOf(taker1.address, yesId);

            await marketExecutionFacet
                .connect(taker1)
                .fillLimitOrders(takerOrderId, [complementaryMakerOrderId, mintMakerOrderId]);

            const takerYesBalanceAfter = await erc1155.balanceOf(taker1.address, yesId);

            // Should have received tokens from both matches
            expect(takerYesBalanceAfter.sub(takerYesBalanceBefore)).to.equal(amount.mul(2));
        });
    });
    ...
    describe("Boundary Condition Tests", function () {
        ...
        it("should handle price boundaries correctly", async () => {
            ...
            await expect(
                marketExecutionFacet.connect(taker1).fillLimitOrders(maxPriceTakerOrderId, [maxPriceMakerOrderId])
            ...
            await expect(
                marketExecutionFacet.connect(taker2).fillLimitOrders(minPriceTakerOrderId, [minPriceMakerOrderId])
            ).to.not.be.reverted;
        });
        ...
        it("should handle timestamp boundaries for expiry", async () => {
            ...
            await expect(
                marketExecutionFacet.connect(taker1).fillLimitOrders(takerOrderId, [exactExpiryOrderId])
            ).to.not.be.reverted;
        });
    });
    ...
    describe("Recovery and Resilience Tests", function () {
        it("should handle partial system failures gracefully", async () => {
            ...
            await erc1155.connect(maker2).safeTransferFrom(maker2.address, owner.address, yesId, maker2Balance, "0x");
            ...
            await expect(
                marketExecutionFacet.connect(taker1).fillLimitOrders(takerOrderId, [validMakerOrderId, insufficientMakerOrderId])
        ...
        it("should maintain data integrity after failed transactions", async () => {
            ...
            const initialTakerOrder = await exchangeFacet.getOrder(takerOrderId);
            const initialMakerOrder = await exchangeFacet.getOrder(makerOrderId);
            const initialTakerBalance = await erc1155.balanceOf(taker1.address, yesId);
            const initialMakerBalance = await erc1155.balanceOf(maker1.address, yesId);

            // Try to execute with invalid order ID (should fail)
            try {
                await marketExecutionFacet.connect(taker1).fillLimitOrders(takerOrderId, [999999]);
            } catch (error) {
                // Expected to fail
            }
            ...
            const afterFailTakerBalance = await erc1155.balanceOf(taker1.address, yesId);
            const afterFailMakerBalance = await erc1155.balanceOf(maker1.address, yesId);
            ...
            await marketExecutionFacet.connect(taker1).fillLimitOrders(takerOrderId, [makerOrderId]);
