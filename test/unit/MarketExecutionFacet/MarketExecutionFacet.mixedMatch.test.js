...
describe("Market Execution Facet - Mixed Matched tests", function () {
    ...
    let diamondAddress, matchExecutionFacet, routeSimFacet, exchangeFacet, erc20, ercUnit, erc1155, conditionalFacet, conditionManagerFacet, adminConfig;
    let questionId, conditionId, yesId, noId;
    let mintAmount, unit, erc20Decimals = 6;
    ...
    before(async function () {
        ...
        matchExecutionFacet = await ethers.getContractAt("MarketExecutionFacet", diamondAddress);
    ...
    it("should execute a BUY market order using Mint and Complementary match routes", async () => {
        ...
        const firstBuyYesLimitOrderPrice = ethers.utils.parseUnits("0.3", erc20Decimals); // Maker wants to buy Yes at 0.3 - Taker pays 0.7 per No Unit - Mint
        const secondSellNoLimitOrderPrice = ethers.utils.parseUnits("0.4", erc20Decimals); // Maker wants to sell No at 0.4 - Taker pays 0.4 per No unit - Comp

        const firstBuyYesLimitOrderAmount = ethers.utils.parseUnits("4", erc20Decimals);
        const secondSellNoLimitOrderAmount = ethers.utils.parseUnits("2", erc20Decimals);

        const marketBuyNoFillAmount = ethers.utils.parseUnits("4", erc20Decimals); // Taker gonna buy No, Fill 2 of them using best 

        const firstOrderCost = firstBuyYesLimitOrderAmount.mul(firstBuyYesLimitOrderPrice).div(ercUnit)
        const firstOrderMakerFee = firstOrderCost.mul(feeConfig.makerBps).div(10_000)
        const firstOrderMakerLocked = firstOrderCost.add(firstOrderMakerFee) // Maker is buying so he lock cost + fee upfront

        const secOrderCost = secondSellNoLimitOrderAmount.mul(secondSellNoLimitOrderPrice).div(ercUnit)
        const secondOrderMakerFee = secOrderCost.mul(feeConfig.makerBps).div(10_000)
        const secondOrderReceive = secOrderCost.sub(secondOrderMakerFee) // Maker is selling so he gets cost - fee

        const secondOrderTakerFee = secOrderCost.mul(feeConfig.takerBps).div(10_000)
        const secondOrderTakerCost = secOrderCost.add(secondOrderTakerFee)

        const remainingAmountForMarketOrder = marketBuyNoFillAmount.sub(secondSellNoLimitOrderAmount); // Second order is offering a better price
        
        const firstOrderTakerPrice = ethers.utils.parseUnits("1", erc20Decimals).sub(firstBuyYesLimitOrderPrice); // 1 - 0.3 = 0.7 taker has to pay per unit of no
        const firstOrderMintCost = remainingAmountForMarketOrder.mul(firstOrderTakerPrice).div(ercUnit)
        const firstOrderTakerFee = firstOrderMintCost.mul(feeConfig.takerBps).div(10_000)
        ...
        expect(makerYesAfter.sub(makerYesBefore)).to.equal(remainingAmountForMarketOrder);

        // // ✅ Maker lost full locked collateral (price * amount + fee)
        expect(makerERC20Before.sub(makerERC20After)).to.equal(firstOrderMakerLocked);
    ...
    it("should execute a BUY market order using multiple Mint match orders", async () => {
        ...
        const secondOrderTakerPayout = ethers.utils.parseUnits("1", erc20Decimals).sub(secondBuyPrice).mul(secondBuyAmount).div(ercUnit);
        remainingAmountForMarketOrder = marketFillAmount.sub(secondBuyAmount);
        const firstOrderTakerPayout = ethers.utils.parseUnits("1", erc20Decimals).sub(firstBuyPrice).mul(remainingAmountForMarketOrder).div(ercUnit);
