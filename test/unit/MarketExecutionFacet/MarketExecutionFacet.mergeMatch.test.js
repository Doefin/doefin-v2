...
describe("Market Execution Facet", function () {
    ...
    it("should execute a Sell market order using Merge match route", async () => {
        ...
        const takerCost = ethers.utils.parseUnits("1", erc20Decimals).sub(price).mul(amount).div(ercUnit); // 3.0
        const takerFee = takerCost.mul(feeConfig.takerBps).div(10_000);
        const takerReceive = takerCost.sub(takerFee);

        await sendAndApproveERC1155({ token: erc1155, sender: owner, to: maker, tokenId: noId, spender: diamondAddress, amount: amount })
        await sendAndApproveERC1155({ token: erc1155, sender: owner, to: taker, tokenId: yesId, spender: diamondAddress, amount: amount })
    ...
    it("should execute a Sell market order using multiple Merge match orders", async () => {
        ...
        const worstOrderAmountNotMatched = firstSellAmount.add(secondSellAmount).sub(marketFillAmount);

        // Cost and fee calculations
        const firstOrderFillableAmount = ethers.utils.parseUnits("2", erc20Decimals); // First order won't get fully matched.
        const firstOrderCost = firstOrderFillableAmount.mul(firstSellPrice).div(ercUnit);
        ...
        const secondOrderTakerPayout = ethers.utils.parseUnits("1", erc20Decimals).sub(secondSellPrice).mul(secondSellAmount).div(ercUnit);
        remainingAmountForMarketOrder = marketFillAmount.sub(secondSellAmount);
        const firstOrderTakerPayout = ethers.utils.parseUnits("1", erc20Decimals).sub(firstSellPrice).mul(remainingAmountForMarketOrder).div(ercUnit);
        ...
        // Fund maker
        await sendAndApproveERC1155({ token: erc1155, sender: owner, to: maker, tokenId: yesId, spender: diamondAddress, amount: firstSellAmount.add(secondSellAmount) })
        await sendAndApproveERC1155({ token: erc1155, sender: owner, to: taker, tokenId: noId, spender: diamondAddress, amount: marketFillAmount })
        ...
        expect(makerYesBefore.sub(worstOrderAmountNotMatched)).to.equal(marketFillAmount); // the rest is still lock in the order.
        expect(makerERC20After.sub(makerERC20Before)).to.equal(makerRecieve);
        expect(takerERC20After.sub(takerERC20Before)).to.equal(takerReceive);
    });
