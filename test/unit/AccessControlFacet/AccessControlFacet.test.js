...
describe("AccessControlFacet", function () {
    ...
    before(async function () {
        [owner, user1, user2, user3, marketMaker1, marketMaker2] = await ethers.getSigners();
    ...
    describe("Market Maker Management", function () {
        it("should allow owner to add market maker", async () => {
            ...
            expect(await accessControlFacet.isMarketMaker(marketMaker1.address)).to.be.true;
        });

        it("should allow owner to remove market maker", async () => {
            // First add market maker
            await accessControlFacet.connect(owner).addMarketMaker(marketMaker1.address);
            expect(await accessControlFacet.isMarketMaker(marketMaker1.address)).to.be.true;

            // Then remove
            await expect(
                accessControlFacet.connect(owner).removeMarketMaker(marketMaker1.address)
            ).to.emit(accessControlFacet, "MarketMakerStatusUpdated")
            .withArgs(marketMaker1.address, false);

            expect(await accessControlFacet.isMarketMaker(marketMaker1.address)).to.be.false;
        });
        ...
        it("should revert if non-owner tries to remove market maker", async () => {
            ...
            await expect(
                accessControlFacet.connect(user1).removeMarketMaker(marketMaker1.address)
            ).to.be.revertedWith("NotContractOwner()");
        });

        it("should revert when adding zero address as market maker", async () => {
            await expect(
                accessControlFacet.connect(owner).addMarketMaker(ethers.constants.AddressZero)
            ).to.be.revertedWith("InvalidMakerAddress()");
        });

        it("should revert when removing zero address as market maker", async () => {
            await expect(
                accessControlFacet.connect(owner).removeMarketMaker(ethers.constants.AddressZero)
            ).to.be.revertedWith("InvalidMakerAddress()");
        });
        ...
        it("should handle multiple market makers", async () => {
            ...
            await accessControlFacet.connect(owner).addMarketMaker(marketMaker1.address);
            await accessControlFacet.connect(owner).addMarketMaker(marketMaker2.address);

            expect(await accessControlFacet.isMarketMaker(marketMaker1.address)).to.be.true;
            expect(await accessControlFacet.isMarketMaker(marketMaker2.address)).to.be.true;
            expect(await accessControlFacet.isMarketMaker(user1.address)).to.be.false;

            // Remove one market maker
            await accessControlFacet.connect(owner).removeMarketMaker(marketMaker1.address);
    ...
    describe("Access Control Integration", function () {
        it("should work with other facets requiring market maker role", async () => {
            const conditionManagerFacet = await ethers.getContractAt("ConditionManagerFacet", diamondAddress);
    ...
    describe("Edge Cases and Security", function () {
        ...
        it("should maintain state consistency across multiple operations", async () => {
            const addresses = [marketMaker1.address, marketMaker2.address, user1.address, user2.address];
            ...
            for (let i = 0; i < addresses.length; i += 2) {
                ...
                expect(await accessControlFacet.isMarketMaker(addresses[i])).to.be.false;
            }
        ...
        it("should handle contract address as market maker", async () => {
            ...
            expect(await accessControlFacet.isMarketMaker(diamondAddress)).to.be.false;
        });
    });
    ...
    describe("Gas Optimization", function () {
        it("should efficiently handle large number of market makers", async () => {
            ...
            console.log(`Time for ${numMarketMakers} market makers operations: ${endTime - startTime}ms`);
        ...
        it("should have consistent gas costs for market maker operations", async () => {
            ...
            for (const addr of addresses) {
                ...
                const addTx = await accessControlFacet.connect(owner).addMarketMaker(addr);
                const addReceipt = await addTx.wait();
                addGasCosts.push(addReceipt.gasUsed);

                // Measure remove gas cost
                const removeTx = await accessControlFacet.connect(owner).removeMarketMaker(addr);
                const removeReceipt = await removeTx.wait();
                removeGasCosts.push(removeReceipt.gasUsed);
            }

            // Gas costs should be relatively consistent
            const addGasVariance = Math.max(...addGasCosts) - Math.min(...addGasCosts);
            const removeGasVariance = Math.max(...removeGasCosts) - Math.min(...removeGasCosts);

            console.log("Add gas costs:", addGasCosts.map(g => g.toString()));
            console.log("Remove gas costs:", removeGasCosts.map(g => g.toString()));
            ...
            const avgAddGas = addGasCosts.reduce((sum, cost) => sum.add(cost), ethers.constants.Zero).div(addGasCosts.length);
            const avgRemoveGas = removeGasCosts.reduce((sum, cost) => sum.add(cost), ethers.constants.Zero).div(removeGasCosts.length);

            expect(addGasVariance).to.be.lessThan(avgAddGas.div(10)); // Less than 10% variance
            expect(removeGasVariance).to.be.lessThan(avgRemoveGas.div(10)); // Less than 10% variance
        });
    });
    ...
    describe("Event Verification", function () {
        it("should emit events with correct parameters", async () => {
            ...
            const addTx = await accessControlFacet.connect(owner).addMarketMaker(marketMaker1.address);
            const addReceipt = await addTx.wait();
            
            const addEvent = addReceipt.events.find(e => e.event === "MarketMakerStatusUpdated");
            expect(addEvent).to.not.be.undefined;
            expect(addEvent.args.marketMaker).to.equal(marketMaker1.address, true);

            // Test MarketMakerRemoved event
            const removeTx = await accessControlFacet.connect(owner).removeMarketMaker(marketMaker1.address);
            const removeReceipt = await removeTx.wait();
            
            const removeEvent = removeReceipt.events.find(e => e.event === "MarketMakerStatusUpdated");
            expect(removeEvent).to.not.be.undefined;
            expect(removeEvent.args.marketMaker).to.equal(marketMaker1.address, false);
        });
        ...
        it("should emit events in correct order for multiple operations", async () => {
            ...
            for (const addr of addresses) {
                await expect(
                    accessControlFacet.connect(owner).addMarketMaker(addr)
                ).to.emit(accessControlFacet, "MarketMakerStatusUpdated")
                .withArgs(addr, true);
            }
