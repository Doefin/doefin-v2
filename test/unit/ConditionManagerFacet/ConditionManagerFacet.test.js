...
describe("ConditionManagerFacet", function () {
    ...
    describe("Condition Creation", function () {
        it("should create a basic binary condition", async () => {
            ...
            await expect(
                conditionManagerFacet.connect(marketMaker).createCondition(
                    oracle.address,
                    questionId,
                    outcomeSlotCount,
                    ipfsHash
            ...
            .withArgs(expectedConditionId, oracle.address, questionId, outcomeSlotCount, ipfsHash, marketMaker.address);

            // Verify condition exists
            const [conditionOracle, conditionQuestionId, conditionOutcomeSlotCount, conditionMetadataURI] = await conditionManagerFacet.getCondition(expectedConditionId);
        ...
        it("should create condition with multiple outcomes", async () => {
            ...
            const [, , conditionOutcomeSlotCount,] = await conditionManagerFacet.getCondition(expectedConditionId);
            expect(conditionOutcomeSlotCount).to.equal(outcomeSlotCount);
        });
        ...
        it("should create conditions with different oracles", async () => {
            ...
            await conditionManagerFacet.connect(marketMaker).createCondition(
                oracle.address,
                questionId1,
                outcomeSlotCount,
                "ipfs://oracle1"
            ...
            const [oracle1, , ,] = await conditionManagerFacet.getCondition(conditionId1);
            const [oracle2Addr, , ,] = await conditionManagerFacet.getCondition(conditionId2);
        ...
        it("should revert if non-market-maker tries to create condition", async () => {
            ...
            await expect(
                conditionManagerFacet.connect(user1).createCondition(
                    oracle.address,
                    questionId,
                    outcomeSlotCount,
                    "ipfs://unauthorized"
    ...
    describe("Condition Queries", function () {
        ...
        it("should return correct condition details", async () => {
            const [conditionOracle, conditionQuestionId, conditionOutcomeSlotCount, conditionMetadataURI] = await conditionManagerFacet.getCondition(testConditionId);
        ...
        it("should verify condition exists by checking oracle address", async () => {
            ...
            const [conditionOracle, , ,] = await conditionManagerFacet.getCondition(testConditionId);
        ...
        it("should return empty data for non-existent condition", async () => {
            ...
            const [conditionOracle, conditionQuestionId, conditionOutcomeSlotCount, conditionMetadataURI] = await conditionManagerFacet.getCondition(nonExistentId);
        ...
        it("should handle condition ID edge cases", async () => {
            ...
            const [oracle1, questionId1, outcomeSlotCount1, metadataURI1] = await conditionManagerFacet.getCondition(maxConditionId);
            expect(oracle1).to.equal(ethers.constants.AddressZero);

            // Test with zero condition ID - should return default values
            const [oracle2, questionId2, outcomeSlotCount2, metadataURI2] = await conditionManagerFacet.getCondition(ethers.constants.HashZero);
    ...
    describe("Condition Cancellation", function () {
        ...
        it("should allow creator to cancel condition", async () => {
            await expect(
                conditionManagerFacet.connect(marketMaker).cancelCondition(testConditionId)
        ...
        it("should revert when canceling non-existent condition", async () => {
            const nonExistentId = ethers.utils.id("non-existent");
            await expect(
                conditionManagerFacet.connect(marketMaker).cancelCondition(nonExistentId)
            ).to.be.revertedWith("ConditionDoesNotExist()");
        });

        it("should revert when canceling already cancelled condition", async () => {
            // First cancellation
            await conditionManagerFacet.connect(marketMaker).cancelCondition(testConditionId);
    ...
    describe("Multiple Conditions Management", function () {
        it("should handle multiple conditions from same oracle", async () => {
            ...
            for (const conditionId of conditionIds) {
                const [conditionOracle, , ,] = await conditionManagerFacet.getCondition(conditionId);
                expect(conditionOracle).to.not.equal(ethers.constants.AddressZero);
            }

            // Cancel some conditions
            await conditionManagerFacet.connect(marketMaker).cancelCondition(conditionIds[0]);
            await conditionManagerFacet.connect(marketMaker).cancelCondition(conditionIds[2]);

            // Verify cancellation doesn't affect other conditions
            const [oracle1, , ,] = await conditionManagerFacet.getCondition(conditionIds[1]);
            expect(oracle1).to.equal(oracle.address);
        });
        ...
        it("should handle conditions from multiple oracles", async () => {
            ...
            const conditionId1 = getConditionId(oracle.address, questionId, outcomeSlotCount);
            const conditionId2 = getConditionId(oracle2.address, questionId, outcomeSlotCount);
    ...
    describe("Edge Cases and Security", function () {
        it("should handle very large outcome slot counts", async () => {
            ...
            const conditionId = getConditionId(oracle.address, questionId, largeOutcomeCount);
            const [, , conditionOutcomeSlotCount,] = await conditionManagerFacet.getCondition(conditionId);
        ...
        it("should handle rapid condition creation and cancellation", async () => {
            ...
            // Rapid cancellation
            for (const { conditionId } of conditionIds) {
                await conditionManagerFacet.connect(marketMaker).cancelCondition(conditionId);
            }

            // Verify all are cancelled (should revert when trying to cancel again)
            for (const { conditionId } of conditionIds) {
                await expect(
                    conditionManagerFacet.connect(marketMaker).cancelCondition(conditionId)
                ).to.be.revertedWith("ConditionAlreadyInactive()");
            }
        });
        ...
        it("should maintain data integrity under concurrent operations", async () => {
            ...
            for (const questionId of questionIds) {
                const conditionId = getConditionId(oracle.address, questionId, 2);
                const [conditionOracle, conditionQuestionId, conditionOutcomeSlotCount, conditionMetadataURI] = await conditionManagerFacet.getCondition(conditionId);
    ...
    describe("Gas Optimization", function () {
        it("should have consistent gas costs for condition operations", async () => {
            ...
            for (let i = 0; i < numConditions; i++) {
                ...
                const createTx = await conditionManagerFacet.connect(marketMaker).createCondition(
                    oracle.address,
                    questionId,
                    2,
                    `ipfs://gas-test-${i}`
                ...
                const cancelTx = await conditionManagerFacet.connect(marketMaker).cancelCondition(conditionId);
                const cancelReceipt = await cancelTx.wait();
                cancelGasCosts.push(cancelReceipt.gasUsed);
            }

            console.log("Create gas costs:", createGasCosts.map(g => g.toString()));
            console.log("Cancel gas costs:", cancelGasCosts.map(g => g.toString()));
            ...
            const createVariance = Math.max(...createGasNumbers) - Math.min(...createGasNumbers);
            const cancelVariance = Math.max(...cancelGasNumbers) - Math.min(...cancelGasNumbers);

            const avgCreateGas = createGasNumbers.reduce((sum, cost) => sum + cost, 0) / createGasNumbers.length;
            const avgCancelGas = cancelGasNumbers.reduce((sum, cost) => sum + cost, 0) / cancelGasNumbers.length;
