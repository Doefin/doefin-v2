...
const { createCompleteMarket, createMarketScenario, measureGas, increaseTime } = require("../utils/marketUtils.js");
...
describe("Full Market Lifecycle Integration Tests", function () {
    ...
    before(async function () {
        [owner, oracle, trader1, trader2, trader3, marketMaker1, marketMaker2] = await ethers.getSigners();
        ...
        await contracts.accessControlFacet.connect(owner).addMarketMaker(marketMaker1.address);
    ...
    describe("Complete Market Creation and Trading Lifecycle", function () {
        it("should handle full lifecycle: creation → trading → resolution → redemption", async () => {
            ...
            const marketMetadata = await contracts.marketDataFacet.getMarketMetadata(market.yesId);
            ...
            // Fund trader1
            const estimatedCost = tradeAmount.mul(maxPrice).div(ethers.utils.parseEther("1"));
            ...
            const trader1BalanceBefore = await contracts.erc20.balanceOf(trader1.address);
            const trader1YesBalanceBefore = await contracts.erc1155.balanceOf(trader1.address, market.yesId);
            ...
            const trader1BalanceAfter = await contracts.erc20.balanceOf(trader1.address);
            const trader1YesBalanceAfter = await contracts.erc1155.balanceOf(trader1.address, market.yesId);

            expect(trader1BalanceBefore.gt(trader1BalanceAfter)).to.be.true;
            expect(trader1YesBalanceAfter.sub(trader1YesBalanceBefore)).to.equal(tradeAmount);
            ...
            await contracts.marketExecutionFacet.connect(trader2).fillMarketOrderWithRoute(
                market.yesId,
            ...
            await contracts.conditionalFacet.connect(oracle).reportPayouts(questionId, payouts);

            // Verify condition is resolved
            const conditionAfterResolution = await contracts.conditionManagerFacet.getCondition(market.conditionId);
            ...
            const trader1ERC20BeforeRedemption = await contracts.erc20.balanceOf(trader1.address);
            const trader1YesBeforeRedemption = await contracts.erc1155.balanceOf(trader1.address, market.yesId);
            ...
            const trader1ERC20AfterRedemption = await contracts.erc20.balanceOf(trader1.address);
            const trader1YesAfterRedemption = await contracts.erc1155.balanceOf(trader1.address, market.yesId);

            // Verify redemption (should receive ERC20 minus resolution fee)
            expect(trader1ERC20AfterRedemption.gt(trader1ERC20BeforeRedemption)).to.be.true;
            expect(trader1YesAfterRedemption.lt(trader1YesBeforeRedemption)).to.be.true;

            // Trader2 tries to redeem losing NO positions (should get nothing)
            const trader2ERC20BeforeRedemption = await contracts.erc20.balanceOf(trader2.address);
            const trader2NoBeforeRedemption = await contracts.erc1155.balanceOf(trader2.address, market.noId);
            ...
            if (trader2NoBeforeRedemption.gt(0)) {
                ...
                const trader2ERC20AfterRedemption = await contracts.erc20.balanceOf(trader2.address);
                // Should receive nothing for losing positions
                expect(trader2ERC20AfterRedemption).to.equal(trader2ERC20BeforeRedemption);
            }
        ...
        it("should handle market with high trading volume and multiple participants", async () => {
            ...
            for (let i = 0; i < traders.length; i++) {
                ...
                if (direction === 0) { // BUY
                    const cost = tradeAmount.mul(ethers.utils.parseEther("0.6")).div(ethers.utils.parseEther("1"));
                    ...
                    await contracts.erc20.connect(trader).approve(diamondAddress, totalCost);
                } else { // SELL
                    await contracts.erc1155.connect(owner).safeTransferFrom(
                        owner.address,
                        trader.address,
                    ...
                    await contracts.erc1155.connect(trader).setApprovalForAll(diamondAddress, true);
                }
                ...
                const { gasUsed } = await measureGas(
                    contracts.marketExecutionFacet.connect(trader).fillMarketOrderWithRoute(
                        market.yesId,
                        tradeAmount,
                        direction === 0 ? ethers.utils.parseEther("0.7") : ethers.utils.parseEther("0.3"),
                        false,
                        direction,
                        route.matches.map(m => [
                            m.matchedOrderId,
                ...
                tradeResults.push({ trader: i, direction, amount: tradeAmount, gasUsed });
            }
            ...
            const avgGas = tradeResults.reduce((sum, result) => sum.add(result.gasUsed), ethers.constants.Zero).div(3);
            ...
            const finalMetadata = await contracts.marketDataFacet.getMarketMetadata(market.yesId);
    ...
    describe("Error Handling and Edge Cases Integration", function () {
        it("should handle failed trades gracefully without affecting market state", async () => {
            ...
            expect(trader1Balance).to.equal(largeAmount.mul(maxPrice).div(ethers.utils.parseEther("1")));
        });
    ...
    describe("Performance and Scalability Integration", function () {
        it("should handle multiple concurrent markets efficiently", async () => {
            ...
            console.log(`Executed ${numMarkets} trades in ${tradeEndTime - tradeStartTime}ms`);
